# Como se escribe el codigo de `common/`

Dos cosas: como se habla con el procesador, y como se escriben los modulos.
Ninguna de las dos es preferencia estetica -- las dos se eligieron mirando el
codigo maquina que sale.
...
Aqui eso no es academico. Se presupuesta `RDPMC` en 20-30 ciclos; envolverlo en
un `.S` le anade la llamada y los volcados encima, y falsea justo la medida que
`bench/` existe para tomar.
...
### Y conviene COMPROBAR que el intrinseco no esta

La regla dice "intrinseco", y la tentacion cuando uno no aparece es escribir la
excepcion con su justificacion. Ya paso: `msr_win.c` nacio con `rdmsr` en asm en
linea y un parrafo explicando que GCC no tenia intrinseco. Era falso -- mingw-w64
lo trae en `<intrin.h>`, igual que MSVC y Clang --, y el codigo que sale es el
mismo que se escribiria a mano:
...
O sea que la excepcion no hacia falta. Antes de argumentar una, compilar un
fichero de tres lineas y mirar.

## Modulos con dueno, sin llamadas indirectas
...
- **Almacenamiento del llamante.** Lo normal es que un tipo opaco obligue al
  modulo a reservar; aqui no puede (ver [`memory.md`](memory.md)), asi que se
  expone `ring_bytes_needed(n)` y el bufer lo aporta quien llama. Encapsulacion
  sin reserva.
...
Y es lo que hace ese directorio **probable**: un modulo con tipo opaco,
...
Hay dos ejemplos completos y probados en `tests/opaque_probe.c`: uno de cada
forma.
